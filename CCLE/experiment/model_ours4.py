...
def reliable_neighbor_filter(similarity_matrix, attention_scores_for_neighbors, topk=10):
    ...
    for i in range(n):
        k_val = min(topk, n)
        if k_val == 0: continue
        weighted_sim_row = similarity_matrix[i, :] * attention_scores_for_neighbo
        if k_val > 0 and n > 0:
...
class ConstructAdjMatrixWithHomogeneous(nn.Module):
    ...
    def forward(self, filtered_cell_kernel, filtered_drug_sim, original_cell_drug_adj):
...
class FusionFeature(nn.Module):
    ...
    def fusion_cell_feature(self):
        ...
        for i in range(3):
            mixed_weight_scalar = 0.7 * static_weights[i] + 0.3 * dynamic_weights[:, i].mean()
