...
cell_drug = pd.read_csv(data_dir + "cell_drug_common_binary.csv", index_col=0).to_numpy(dtype=np.float32)
drug_feature = pd.read_csv(data_dir + "drug_feature.csv", index_col=0).to_numpy(dtype=np.float32)
gene = pd.read_csv(data_dir + "cell_gene_feature.csv", index_col=0).to_numpy(dtype=np.float32)
cna = pd.read_csv(data_dir + "cell_gene_cna.csv", index_col=0).fillna(0).to_numpy(dtype=np.float32)
mutation = pd.read_csv(data_dir + "cell_gene_mutation.csv", index_col=0).to_numpy(dtype=np.float32)
mutation = (mutation != 0).astype(np.float32)
null_mask = pd.read_csv(data_dir + "null_mask.csv", index_col=0).to_numpy(dtype=np.float32)
...
if results_for_plotting:
    ...
    output_filename = "results_for_roc_curves_oursGDSC.csv"
